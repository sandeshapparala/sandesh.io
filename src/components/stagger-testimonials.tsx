...
const TestimonialCard: React.FC<TestimonialCardProps> = ({
    ...
    return (
        <div
            ...
            className={cn(
                "absolute left-1/2 top-1/2 cursor-pointer border-2 p-8 transition-all duration-700 ease-out",
                isCenter
...
export const StaggerTestimonials: React.FC = () => {
    ...
    const handleMove = useCallback((steps: number) => {
        ...
        setTimeout(() => {
            setAnimationInProgress(false);
        }, 800); // Slightly longer than CSS transition duration (700ms)
    }, [testimonialsList, animationInProgress]);

    // Start auto-sliding
    useEffect(() => {
        if (!animationInProgress) {
            autoPlayIntervalRef.current = setInterval(() => {
                // Manually set animation in progress before triggering move
                setAnimationInProgress(true);
    ...
    }, [testimonialsList, handleMove, animationInProgress]);
    ...
    return (
        ...
        <div>
            ...
            <div
                className="relative w-full overflow-hidden bg-background max-w-6xl mx-auto p-8 rounded-lg [mask-image:radial-gradient(600px_circle_at_center,white,transparent)] hover:[mask-image:radial-gradient(700px_circle_at_center,white,transparent)]"
                style={{height: 600}}
            >
