...
const processCardVariants = cva("flex border backdrop-blur-lg", {
  variants: {
    variant: {
      indigo:
        "flex border text-slate-50 dark:[border:1px_solid_rgba(255,255,255,.1)] backdrop-blur-lg group-hover:bg-black/[.03] group-hover:dark:bg-neutral-800/10",
      light: "shadow",
    },
    size: {
...
export const ContainerScroll = ({
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <div
        ref={scrollRef}
        className={cn("relative min-h-[250vh]", className)}
        {...props}
      >
        {children}
      </div>
